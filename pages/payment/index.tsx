  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-24 gap-8 bg-background">
      ...
      <form onSubmit={onSubmit}>
        <fieldset>
          <div className="fieldset relative flex flex-col md:flex-row">
            <label htmlFor="card_num">Card Number</label>
            <input
              type="text"
              id="card_num"
              {...register("card_num")}
              className={`w-full ${errors.card_num && "border-red-500"}`}
              placeholder="1234 5678 9012 3456"
              onChange={handleCardNumberChange}
            />
            <p className="uppercase absolute right-3 top-1/2 -translate-y-1/2 font-bold">
              {cardName}
            </p>
          </div>
          {errors.card_num && (
            <p className="text-xs text-red-600">{errors.card_num.message}</p>
          )}
        </fieldset>

        <div className="flex items-center flex-col sm:flex-row gap-4">
          <fieldset className="flex-1">
            <div className="fieldset flex flex-col md:flex-row">
        ...
        <input
          type="submit"
          value="Purchase"
          className="cursor-pointer p-2 px-3 border border-border rounded-sm font-semibold hover:bg-accent"
        />
      </form>
    </div>
  );
